...
const validateDistrictFields = (row: Record<string, unknown>): boolean => {
  return (
...
const isDistrictData = (value: unknown): boolean => {
  ...
  return validateDistrictFields(value as Record<string, unknown>);
};
...
const isStaticMapsDataSource = (
  ...
  return ((value as Record<string, unknown>)['districts'] as unknown[]).every(
    isDistrictData
  );
};
