...
Navigation links: team, go to the map, project overview, subprojects (axis?), open positions (join our team), news, opportunities (scholarships), contact,

Home page: remove contractions
...
`src/app/(internal)/` is a non-public area for UI/UX experiments. Rules:

- Every page under `(internal)` must export `export const metadata = { robots: { index: false, follow: false } }` or use a `<meta name="robots" content="noindex">` equivalent.
- No link anywhere in the app (nav, footer, page copy, CTAs) may point to any `(internal)` route.
- Never add `(internal)` routes to sitemaps or navigation config.
